...
st.sidebar.title("CRisk: Credit Risk Prediction using Fuzzy ML")
st.sidebar.header("📌 Navigation")

page = st.sidebar.selectbox("Select a page", ["Credit Risk Analysis", "EPS", "About"])

if page == "Credit Risk Analysis":
    st.title("Credit Risk Analysis and Prediction (Fuzzy version)")
    ...
    if uploaded_file1 and uploaded_file2 and uploaded_unseen:
        with st.spinner("Program is running..."):
            ...
            # Define class risk levels
            risk_levels = [0.0, 0.3, 0.7, 1.0]

            # Calculate fuzzy risk scores
            # fuzzy_risk_scores = np.dot(y_pred_proba, risk_levels)

            # # Display fuzzy risk scores for the test set
            # st.write("Fuzzy Risk Scores for Test Set")
            # st.dataframe(pd.DataFrame({
            #     "Actual Class": label_encoder.inverse_transform(y_test),
            #     "Fuzzy Risk Score": fuzzy_risk_scores
            # }))
            ...
            y_pred_unseen_proba = xgb_classifier.predict_proba(df_unseen_encoded)

            fuzzy_risk_scores_unseen = np.dot(y_pred_unseen_proba, risk_levels)

            
            fuzzy_scores_columns = ['P1', 'P2', 'P3', 'P4']
            fuzzy_scores_df_unseen = pd.DataFrame(y_pred_unseen_proba, columns=fuzzy_scores_columns)
            df_unseen = pd.concat([df_unseen.reset_index(drop=True), fuzzy_scores_df_unseen.reset_index(drop=True)], axis=1)

            df_unseen['Risk Score'] = fuzzy_risk_scores_unseen
            
            # Define a function to assign Risk Level based on the Risk Score
            ...
            # Apply the function to assign Risk Levels for each row
            df_unseen['Risk Level'] = df_unseen['Risk Score'].apply(assign_risk_level)

            risk_level_counts = df_unseen['Risk Level'].value_counts()

            # Plot the bar chart
            plt.figure(figsize=(8, 6))
            risk_level_counts.plot(kind='bar', color='skyblue')
